# P.E.O - Gerador de Instalador MODO INTRANET
# Este instalador inclui cadastro por e-mail e suporte a rede

$ErrorActionPreference = "Stop"

Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  P.E.O - BUILD INSTALADOR MODO INTRANET   " -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""

# Define variavel de ambiente para o modo
$env:PEO_MODE = "INTRANET"
...
Write-Host "[5/5] Gerando instalador .exe MODO INTRANET..." -ForegroundColor Blue
npm run build:win
if ($LASTEXITCODE -ne 0) { 
...
$arquivoOriginal = Get-ChildItem -Path "dist" -Filter "*.exe" | Select-Object -First 1
if ($arquivoOriginal) {
    $novoNome = "P.E.O - Intranet Setup 1.0.0.exe"
    Rename-Item -Path $arquivoOriginal.FullName -NewName $novoNome -Force
    Write-Host ""
    Write-Host "============================================" -ForegroundColor Green
    Write-Host "[OK] INSTALADOR INTRANET GERADO COM SUCESSO!" -ForegroundColor Green
    Write-Host "============================================" -ForegroundColor Green
    Write-Host "[>>] Localizacao: dist\$novoNome" -ForegroundColor White
    Write-Host ""
} else {
    Write-Host "[!] Instalador gerado mas nao renomeado automaticamente." -ForegroundColor Yellow
}
