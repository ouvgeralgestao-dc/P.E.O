...
db.serialize(() => {
    db.all(query, (err, rows) => {
        ...
        if (rows.length === 0) {
            console.log("No sectors found for 'Comunicação Social'. Dumping all distinct organ names:");
            db.all("SELECT DISTINCT nome FROM orgaos", (err, orgs) => {
                orgs.forEach(o => console.log(`- ${o.nome}`));
            });
        }
    });
});
